...
coke_plot <- ggplot(df, aes(x = df$taste_cola, y = df$hours_music_per_week, colour = df$taste_cola)) +
  theme_minimal() +
    labs(x = "Coke", y = "Hours Music Per Week") 
coke_plot +
  geom_bar(stat = 'summary', fun.y = mean, width = 0.25, fill = 'white')
```
...
```{r}
clean_hoursmusic <- filter(df, df$hours_music_per_week<30)

coke_plot_new <- ggplot(clean_hoursmusic, aes(x = clean_hoursmusic$taste_cola, y = clean_hoursmusic$hours_music_per_week, colour = clean_hoursmusic$taste_cola)) +
  theme_minimal() +
    labs(x = "Coke", y = "Hours Music Per Week") 
coke_plot_new +
  geom_bar(stat = 'summary', fun.y = mean, width = 0.25, fill = 'white')
...
```{r}
#filter data by 3 standard deviations of the mean 

#df$zscore <- (df$balloon_balance-mean(df$balloon_balance))/sd(df$balloon_balance)
```
...
ggplot(clean_hoursmusic, aes(x = hours_music_per_week)) + #note that I use 'filtered' dataframe here :) It might be called differently for you
  geom_histogram(aes(y = ..density..), binwidth = 0.25) +
  ggtitle("This is what music time looks like without the biggest outlier") +
  stat_function(fun = dnorm, args = list(mean = mean(clean_hoursmusic$hours_music_per_week, na.rm = TRUE), sd = sd(clean_hoursmusic$hours_music_per_week, na.rm = TRUE)), colour= "darkgreen", size = 1)+
  theme_classic()
...
qqnorm(clean_hoursmusic$hours_music_per_week)
```
